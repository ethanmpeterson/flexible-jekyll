...
module-by-module breakdown is provided in section 3.2 of the full report
...
ADCs respectively. Each module outputs the data on an AXI streams. AXI streams
...
samples across the various clock domains until they can be encoded and sent over
USB. The FTDI module takes an AXI stream as input and converts it to the
parallel interface of the FT232H.
...
Given below is a sequence diagram for the desktop application.
...
Competition and won both the department and IEE awards. TeachEE won first place
for both the ECE department and the Kingston IEEE chapter. Below is a gallery of
photos from the capstone showcase.
